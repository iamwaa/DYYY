...
// 十六进制转 UIColor，解析失败返回 nil
static UIColor *WaaColorFromHexString(NSString *hex) {
    if (hex.length == 0) {
        return nil;
    }
...
// 评论区自定义背景色，未开启或未填写时返回 nil
static UIColor *WaaCommentBackgroundColor(void) {
    if (!DYYYGetBool(@"WaaEnableCommentBgColor")) {
        return nil;
    }
    return WaaColorFromHexString(DYYYGetString(@"WaaCommentBgColor"));
}
...
- (void)setBackgroundColor:(UIColor *)backgroundColor {
    ...
    if ((isTargetCommentContainer || isFirstChildOfCommentContainer) && !DYYYGetBool(@"DYYYEnableCommentBlur")) {
        ...
        // 背景改色与透明度作用于同一个评论区视图，先换色再统一取透明度
        UIColor *customBgColor = WaaCommentBackgroundColor();
        if (customBgColor) {
...
static id WaaColorizedYYText(id text) {
    ...
    NSString *digits = [hex hasPrefix:@"#"] ? [hex substringFromIndex:1] : hex;
    NSScanner *scanner = [NSScanner scannerWithString:digits ?: @""];
    if (hex.length == 0 || ![scanner scanHexInt:&value]) return text;
...
// setTextLayout: 定义在父类 YYLabel 上，运行时替换会影响全 App 的 YYLabel（如视频文案）
// 因此必须在实现里按实例类名限定，仅处理评论正文
static BOOL WaaIsCommentBodyLabel(id object) {
    return [NSStringFromClass([object class]) isEqualToString:@"AWECommentPanelListSwiftImpl.BaseCellCommentLabel"];
}

static void WaaYYLabelSetTextLayout(id object, SEL selector, id layout) {
    id layoutToSet = layout;
    if (WaaIsCommentBodyLabel(object) && DYYYGetBool(@"WaaEnableCommentColor") && layout) {
        id text = nil;
        id container = nil;
        @try {
            text = [layout valueForKey:@"text"];
            container = [layout valueForKey:@"container"];
        } @catch (NSException *exception) {
            text = nil;
            container = nil;
        }

        id colorizedText = WaaColorizedYYText(text);
        Class layoutClass = NSClassFromString(@"YYTextLayout");
        SEL factorySelector = NSSelectorFromString(@"layoutWithContainer:text:");
        Method factoryMethod = layoutClass ? class_getClassMethod(layoutClass, factorySelector) : NULL;
        if (colorizedText != text && container && factoryMethod) {
            id (*factory)(id, SEL, id, id) =
                (id (*)(id, SEL, id, id))method_getImplementation(factoryMethod);
            id replacement = factory ? factory(layoutClass, factorySelector, container, colorizedText) : nil;
            if (replacement) layoutToSet = replacement;
        }
    }

    if (gWaaOriginalYYLabelTextLayoutSetter) {
        gWaaOriginalYYLabelTextLayoutSetter(object, selector, layoutToSet);
    }
}
...
static void WaaInstallYYLabelTextLayoutSetterIfNeeded(void) {
    ...
    SEL selector = NSSelectorFromString(@"setTextLayout:");
    Method method = labelClass ? class_getInstanceMethod(labelClass, selector) : NULL;
    if (!method) return;

    gWaaOriginalYYLabelTextLayoutSetter =
        (void (*)(id, SEL, id))method_setImplementation(method, (IMP)WaaYYLabelSetTextLayout);
    installed = gWaaOriginalYYLabelTextLayoutSetter != NULL;
}

static void WaaCustomYYLabelSetAttributedText(id object, SEL selector, id text) {
    id textToSet = text;
    if (WaaIsCommentBodyLabel(object) &&
        [text isKindOfClass:[NSAttributedString class]] && [text length] > 0 &&
...
- (void)applyAudioCommentColorRecursive:(UIView *)view customColor:(UIColor *)customColor;
@end
...
// 语音评论：图标、波形 CAShapeLayer 和时长文字，统一用比正文暗一档的颜色
- (void)applyAudioCommentColorRecursive:(UIView *)view customColor:(UIColor *)customColor {
    if ([view isKindOfClass:[UILabel class]]) {
        ((UILabel *)view).textColor = customColor;
    ...
    // AWECommentShapeView 的波形由 CAShapeLayer 绘制，需同时写 stroke 与 fill
    if ([view.layer isKindOfClass:[CAShapeLayer class]]) {
        CAShapeLayer *shapeLayer = (CAShapeLayer *)view.layer;
        if (shapeLayer.strokeColor) {
            shapeLayer.strokeColor = customColor.CGColor;
        }
    ...
    for (UIView *subview in view.subviews) {
        [self applyAudioCommentColorRecursive:subview customColor:customColor];
    }
}
...
- (void)layoutSubviews {
    ...
    WaaInstallYYLabelTextLayoutSetterIfNeeded();
    ...
    if (isCommentColorEnabled) {
        ...
        if (customColor) {
            ...
            // 语音评论属于非正文，用暗一档的颜色
            if ([className isEqualToString:@"AWECommentAudioContentView"]) {
                [self applyAudioCommentColorRecursive:self customColor:darkerColor];
            }
